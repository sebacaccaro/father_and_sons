...
with open("deputati_I.json") as f:
    deputati = json.load(f)

dep_surnames = [dep["surname"] for dep in deputati]
dep_surnames = [d["surname"] if dep_surnames.count(
    d["surname"]) == 1 else f'{d["surname"]} {d["name"]}'.upper() for d in deputati]
...
def cutIndex(doc, indexStr="lasedutacomincia"):
    ...
    while cut == -1 and i < len(doc):
        ...
        if similar(line[:len(indexStr)], indexStr) > 0.6:
            cut = i
        i += 1
    if i == len(doc):
...
def cutIntoLinesSpeeches(lines):
    ...
    speeches = [{"speaker": a["speaker"], "text":lines[a["startPos"]
        :b["startPos"]]} for a, b in pairwise(indexes)]
    return speeches
...
i = 0
speeches = []
for doc in tqdm(data, desc="Parsing first legislature"):
    speeches = [*speeches, *getDocSpeeches(doc["text"])]

with open('parsed_1.json', 'w') as outfile:
